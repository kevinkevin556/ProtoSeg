...
class DefaultLoader(data.Dataset):
    def __init__(
        self, root_dir, aug_transform=None, dataset=None, img_transform=None, label_transform=None, configer=None
    ):
    ...
    def __getitem__(self, index):
        ...
        labelmap = ImageHelper.read_image(
            self.label_list[index], tool=self.configer.get("data", "image_tool"), mode="P"
        )
        ...
        # ori_target[ori_target == 255] = -1    -> Since ori_target.dtype == np.uint8, this line does nothing to the ori_target.
        ...
        meta = dict(ori_img_size=img_size, border_size=border_size, ori_target=ori_target)
        return dict(
...
class CSDataTestLoader(data.Dataset):
    ...
    def __list_dirs(self, root_dir, dataset):
        ...
        if (
            self.configer.get("dataset") == "cityscapes"
            or self.configer.get("dataset") == "camvid"
            or self.configer.get("dataset") == "autonue21"
        ):
