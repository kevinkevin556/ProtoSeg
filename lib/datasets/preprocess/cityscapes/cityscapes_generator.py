...
class CityscapesGenerator(object):
    ...
    def __init__(self, args, image_dir=IMAGE_DIR, label_dir=LABEL_DIR):
        ...
        print("train:")
        print("label_dir:", self.train_label_dir)
        print("image_dir:", self.train_image_dir, "\n")
        print("val:")
        print("label_dir:", self.val_label_dir)
        print("image_dir:", self.val_image_dir, "\n")
        print("coarse:")
        print("label_dir:", self.coarse_label_dir)
        print("image_dir:", self.coarse_image_dir, "\n")

    def generate_label(self):
        print("Generate labels from:", self.args.ori_root_dir, "...")
        ...
        if not self.args.coarse:
            ...
            for image_file in tqdm(self.__list_dir(ori_train_img_dir)):
            ...
            for image_file in tqdm(self.__list_dir(ori_val_img_dir)):
                image_name = "_".join(image_file.split("_")[:-1])
                label_file = "{}_gtFine_labelIds.png".format(image_name)
                shotname, extension = os.path.splitext(image_file.split("/")[-1])
                shutil.copy(
            ...
            for image_file in tqdm(self.__list_dir(ori_train_extra_img_dir)):
                image_name = "_".join(image_file.split("_")[:-1])
                label_file = "{}_gtCoarse_labelIds.png".format(image_name)
                shotname, extension = os.path.splitext(image_file.split("/")[-1])
                shutil.copy(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--coarse", type=str2bool, nargs="?", default=False, dest="coarse", help="Whether is the coarse data."
    )
    parser.add_argument("--save_dir", default=None, type=str, dest="save_dir", help="The directory to save the data.")
    parser.add_argument(
        "--ori_root_dir", default=None, type=str, dest="ori_root_dir", help="The directory of the cityscapes data."
    )
