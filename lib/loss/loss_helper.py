...
from lib.loss.lovasz_loss import flatten_probas, lovasz_softmax_flat
...
class FSCELOVASZLoss(nn.Module):
    ...
    def forward(self, inputs, *targets, weights=None, **kwargs):
        ...
        if isinstance(inputs, tuple) or isinstance(inputs, list):
            ...
            loss_lovasz = lovasz_softmax_flat(*flatten_probas(pred, target, self.ignore_index), only_present=True)
...
class WeightedFSOhemCELoss(nn.Module):
    ...
    def forward(self, predict, target, min_kept=1, weight=None, ignore_index=-1, **kwargs):
        ...
        loss_matrix = (
            F.cross_entropy(predict, target, weight=weight, ignore_index=ignore_index, reduction="none")
            .contiguous()
            .view(
                -1,
            )
...
class MSFSAuxRMILoss(nn.Module):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        scaled_pred_05x = torch.nn.functional.interpolate(
            pred_05x, size=(seg_out.size(2), seg_out.size(3)), mode="bilinear", align_corners=False
        )
...
class SegFixLoss(nn.Module):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        gt_direction = DTOffsetHelper.angle_to_direction_label(
            angle_map, seg_label_map=seg_label_map, extra_ignore_mask=(binary_pred_mask == 0), return_tensor=True
        )
