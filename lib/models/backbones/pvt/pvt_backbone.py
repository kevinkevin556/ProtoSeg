...
class PyramidVisionTransformer(nn.Module):
    def __init__(
        ...
        self.patch_embed2 = PatchEmbed(
            img_size=img_size // patch_size, patch_size=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]
        )
        self.patch_embed3 = PatchEmbed(
            img_size=img_size // patch_size // 2, patch_size=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]
        )
        self.patch_embed4 = PatchEmbed(
            img_size=img_size // patch_size // 4, patch_size=2, in_chans=embed_dims[2], embed_dim=embed_dims[3]
        )
...
class PVTBackbone(object):
    ...
    def __call__(self):
        ...
        model = ModuleHelper.load_model(
            model, pretrained=self.configer.get("network", "pretrained"), all_match=False, network="pvt"
        )
