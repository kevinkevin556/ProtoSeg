...
class GroupBlock(TimmBlock):
    def __init__(
        ...
        super(GroupBlock, self).__init__(
            dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, drop_path, act_layer, norm_layer
        )
        del self.attn
        if ws == 1:
...
class SVTBackbone(object):
    ...
    def __call__(self):
        ...
        model = ModuleHelper.load_model(
            model, pretrained=self.configer.get("network", "pretrained"), all_match=False, network="svt"
        )
