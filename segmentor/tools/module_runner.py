...
class ModuleRunner(object):
    ...
    def save_net(self, net, save_mode="iters", experiment=None):
        ...
        if self.configer.get("checkpoints", "checkpoints_root") is None:
            checkpoints_dir = os.path.join(
                self.configer.get("project_dir"), self.configer.get("checkpoints", "checkpoints_dir")
            )
        else:
            checkpoints_dir = os.path.join(
        ...
        if save_mode == "performance":
            ...
            if self.configer.get("iters") - self.configer.get("last_iters") >= self.configer.get(
                "checkpoints", "save_iters"
            ):
                latest_name = "{}_iters{}.pth".format(
                    self.configer.get("checkpoints", "checkpoints_name"), self.configer.get("iters")
                )
                torch.save(state, os.path.join(checkpoints_dir, latest_name))
                self.configer.update(["last_iters"], self.configer.get("iters"))

        elif save_mode == "epoch":
            if self.configer.get("epoch") - self.configer.get("last_epoch") >= self.configer.get(
                "checkpoints", "save_epoch"
            ):
                latest_name = "{}_epoch{}.pth".format(
                    self.configer.get("checkpoints", "checkpoints_name"), self.configer.get("epoch")
                )
                torch.save(state, os.path.join(checkpoints_dir, latest_name))
    ...
    def freeze_bn(self, net, syncbn=False):
        for m in net.modules():
            ...
            if syncbn:
                from lib.extensions import BatchNorm1d, BatchNorm2d

                if isinstance(m, BatchNorm2d) or isinstance(m, BatchNorm1d):
                    m.eval()

    def clip_grad(self, model, max_grad=10.0):
